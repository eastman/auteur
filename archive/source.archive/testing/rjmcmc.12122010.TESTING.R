...
rjMCMC.bm<-function (phy, data, ngen=1000, sampleFreq=100, probMergeSplit=0.05, probRoot=0.01, lambdaK=log(2), constrainK=FALSE, jumpsize=NULL, fileBase="result", simplestart=FALSE, switcheroo=TRUE, calibrate=TRUE, summary=TRUE, strict.jumpsize=FALSE) 
{ 
# ngen=1000; sampleFreq=100; probMergeSplit=0.05; probRoot=0.01; lambdaK=log(2); constrainK=FALSE; jumpsize=NULL; fileBase="result"; simplestart=FALSE; switcheroo=TRUE; calibrate=FALSE; summary=TRUE
	model="BM"
	heat=1
	require(geiger)
	if(is.null(jumpsize) & calibrate) jumpsize=calibrate.mcmc(phy,data,ngen/100)
	print(paste("Using jumpsize of :", jumpsize,sep=" "))
...
# proposal counts
	...
	tuneFreq=unique(ceiling(5^(1:ceiling(log(ngen,5)))))
	tuneFreq=tuneFreq[tuneFreq<ngen]
	last.propfreq=c(1-(sum(2*probMergeSplit, probRoot)), probMergeSplit, probRoot)
...
## BM IMPLEMENTATION ##
		while(1) {
			if (runif(1) < (2 * last.propfreq[2]) & !constrainK) {			# adjust rate categories
				...
				if(runif(1)<0.5) {											# adjust rates
					...
					if(runif(1)<0.25 & sum(cur.delta.rates)>1 & switcheroo) {	# swap rates classes
	...
	# compare likelihoods
		...
		if(any(tuneFreq%in%i)) {
			print(last.parms)
			print(endparms)
			print(cOK)
			cur.parms=endparms-last.parms
			cur.cOK=cOK-last.cOK
			cur.acceptrates=cur.cOK/cur.parms
			last.acceptrates=last.cOK/last.parms
			comp.ar=cur.acceptrates-last.acceptrates
			cur.propfreq=last.propfreq
			ar.array=c(1,1,2,3)
			if(any(!is.na(comp.ar))) {
				adj.ar=which(comp.ar==min(comp.ar,na.rm=TRUE))
				selector=selector=min(ar.array[adj.ar])
				cur.propfreq[selector]=cur.propfreq[selector]-runif(1, 0, cur.propfreq[selector]/2)
				cur.propfreq[-selector]=cur.propfreq[-selector]/sum(cur.propfreq[-selector])*(1-cur.propfreq[selector])
			}
			print(cur.acceptrates-last.acceptrates)
#			print(last.acceptrates)
			if((any(!is.na(cur.acceptrates)) & any(!is.na(last.acceptrates))) & (mean(cur.acceptrates, na.rm=TRUE) <= mean(last.acceptrates, na.rm=TRUE)) & !strict.jumpsize) jumpsize=adjust.rate(jumpsize, log(2))
			last.propfreq=cur.propfreq
			last.parms=cur.parms
			last.cOK=cur.cOK
			print(paste("Using jumpsize of :", jumpsize,sep=" "))
		}
	}
...
# End rjMCMC
	close(errorLog)
	close(runLog)
	if(summary) summarize.run(cOK, endparms, cur.model)	
...
calibrate.mcmc=function(phy, dat, nsteps, jumpsizes=NULL) {
	print("CALIBRATING jumpsize...")
#	if(nsteps<=10 | nsteps > 1000) nsteps=100
	nsteps=100
	if(is.null(jumpsizes)) jumpsizes=2^(-2:3)
	acceptance.rates=sapply(jumpsizes, function(x) rjMCMC.bm(phy, dat, nsteps, jumpsize=x, summary=FALSE, strict.jumpsize=TRUE))
	return(jumpsizes[which(acceptance.rates==max(acceptance.rates))])
}