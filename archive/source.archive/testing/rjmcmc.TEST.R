...
### prepare data for rjMCMC
	...
	node.anc		<- sapply(unique(c(ape.tre$edge[1,1],ape.tre$edge[,2])), function(x) get.ancestors.of.node(x, ape.tre))
	names(node.des) <- names(node.anc) <- c(ape.tre$edge[1,1], unique(ape.tre$edge[,2]))
...
#	cur.tre			<- apply.BMM.to.apetree(cur.tre, cur.rates)
	vcv				<- update.vcv(ape.tre, node.des, vcv.phylo(ape.tre), rep(1, length(cur.rates)), cur.rates)
	cur.tre			<- vcv$new.tre
	cur.vcv			<- vcv$vcv
	mod.cur = new.bm.lik.fn(cur.rates, cur.root, cur.vcv, orig.dat)
...
#		new.tre=apply.BMM.to.apetree(ape.tre, new.rates)
		if(any(cur.rates!=new.rates)) {
			tmp.vcv=update.vcv(ape.tre, node.des, cur.vcv, cur.rates, new.rates)
			new.vcv=tmp.vcv$vcv
			new.tre=tmp.vcv$new.tre
		} else {
			new.vcv=cur.vcv
			new.tre=cur.tre
		}
		mod.new=try(new.bm.lik.fn(new.rates, new.root, new.vcv, orig.dat),silent=TRUE)
...
new.bm.lik.fn <- function(rates, root, new.vcv, orig.dat) { # from LJ HARMON	
...
update.vcv=function(ape.tre, node.des, cur.vcv, cur.rates, new.rates) {
#	vcv is updated each generations
#	use original tree to multiply branches by rates
	VCV=cur.vcv
	orig<-phy<-ape.tre
	phy$edge.length=phy$edge.length*new.rates
	n=Ntip(phy)
	if(all(new.rates!=cur.rates)) {
		return(list(vcv=vcv.phylo(phy), new.tre=phy)) 
	} else {
		nodes=as.numeric(phy$edge[which(new.rates!=cur.rates),2])
		tips=nodes[nodes<=n]
	}
#	print(node)
#	curnodes=node.des[[which(names(node.des)==node)]]
#	tips=curnodes[curnodes<=n]
	tt=length(tips)
	vcv.block=matrix(0,tt,tt)
	path.branches=function(tip, phy){
		e=0
		nn=tip
		branchset=c()
		while(1) {
			nn=phy$edge[marker<-(which(phy$edge[,2]==nn)),1]
			if(length(nn)) {
				e=e+1
				branchset[e]=marker
			} else {
	...
	branches=lapply(tips, function(x) path.branches(x, phy))
	for(i in 1:nrow(vcv.block)) {
		for(j in 1:ncol(vcv.block)) {
			if(i==j) {
				VCV[tips[i],tips[j]]=sum(phy$edge.length[branches[[i]]])
			} else {
				tmp=sum(phy$edge.length[intersect(branches[[i]],branches[[j]])])
				VCV[tips[j],tips[i]]<-VCV[tips[i],tips[j]]<-tmp
			}
		}
	}
	return(list(vcv=VCV, new.tre=phy))
}
