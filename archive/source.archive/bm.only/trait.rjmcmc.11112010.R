...
rjMCMC.trait<-function (phy, data, ngen=1000, sampleFreq=100, probMergeSplit=0.05, probRoot=0.01, lambdaK=log(2), constrainK=FALSE, fileBase="result", simplestart=FALSE) 
{
	model="BM"
	heat=1
#	require(ouch)
...
# initialize parameters
	...
	if(simplestart | is.numeric(constrainK) ) {
		if(is.numeric(constrainK)) {
			init.rate	<- generate.starting.point(data,ape.tre,node.des,theta=FALSE, K=constrainK)
...
#	namesparms=c("rate.adj", "rate.swp", "rate.cat", "root.adj")
...
## BM IMPLEMENTATION ##
		while(1) {
			if (runif(1) < (2 * probMergeSplit) & !constrainK) {			# adjust rate categories
				nr=split.or.merge(cur.delta.rates, cur.rates, ape.tre, node.des, theta=FALSE, lambdaK)
...
generate.starting.point <- function(data, phy, node.des, theta=FALSE, K=FALSE) { 
