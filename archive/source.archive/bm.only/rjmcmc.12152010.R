...
rjMCMC.bm<-function (phy, data, ngen=1000, sample.freq=100, prob.mergesplit=0.05, prob.root=0.01, lambdaK=log(2), constrainK=FALSE, jumpsize=NULL, simplestart=FALSE, summary=TRUE, fileBase="result") 
{ 
...
# proposal counts
	...
	proposal.rates=c(2*prob.mergesplit, prob.root, 1-sum(c(2*prob.mergesplit, prob.root)))
	prop.cs=cumsum(proposal.rates)
	tickerFreq=ceiling(ngen/30)
	tuneFreq.tmp=unique(ceiling(10^(1:ceiling(log(ngen,10)))))
	tuneFreq=tuneFreq.tmp[tuneFreq.tmp<ngen]
...
#	print("CALIBRATING jumpsize...")
	...
	acceptance.rates=sapply(jumpsizes, function(x) rjMCMC.bm(phy, dat, nsteps, jumpsize=x, summary=FALSE, fileBase="jumpsize")$acceptance.rate)
