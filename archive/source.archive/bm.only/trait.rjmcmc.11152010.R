...
## BM IMPLEMENTATION ##
		while(1) {
			if (runif(1) < (2 * probMergeSplit) & !constrainK) {			# adjust rate categories
				nr=split.or.merge(cur.delta.rates, cur.rates, ape.tre, node.des, theta=FALSE, lambdaK, jumpsize)
				...
				if(runif(1)<probRoot) {										# adjust root
					...
					if(runif(1)>0.1 & sum(cur.delta.rates)!=0) {			# swap rate classes
						nr=switcheroo(cur.delta.rates, cur.rates, ape.tre, node.des, theta=FALSE, jumpsize)
						new.rates=nr$new.values ##
						new.delta.rates=nr$new.delta ##
						new.root=cur.root
...
split.or.merge <- function(cur.delta, cur.values, phy, node.des, theta=FALSE, lambda=lambda, jumpsize=jumpsize) { 
...
#		print("s")
		if(theta) {							# assign new theta to current node
			new.vv[marker] <- nr <- adjust.value(new.vv[marker], jumpsize)
		} else {							# assign new rate to current node
			new.vv[marker] <- nr <- adjust.rate(new.vv[marker], jumpsize)
		}
...
switcheroo <- function(cur.delta, cur.values, phy, node.des, theta=FALSE, jumpsize) { 
	bb=cur.delta
	vv=cur.values
	new.bb=sample(bb)
	...
	# find most ancestral value in tree
		...
		if(theta) anc.value=mean(sapply(old.values, function(x) adjust.value(x, jumpsize))) else anc.value=mean(sapply(old.values, function(x) adjust.rate(x, jumpsize)))
	}
