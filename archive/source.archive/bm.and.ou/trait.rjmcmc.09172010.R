...
rjMCMC.trait<-function (phy, data, ngen=1000, sampleFreq=100, model=c("OU","BM"), probAlpha=0.05, probMergeSplit=0.05, probRoot=0.1, theta.rates.ratio=1, heat=1, fileBase="result") 
{
...
### prepare objects for rjMCMC
	...
	nThetaOK = 0
	ll <- list()											# array for storing output: size is number of MCMC generations
...
### Begin rjMCMC
    for (i in 1:ngen) {
		...
		startparms=list(rates=cur.rates, cats=sum(cur.delta.rates)+1, root=cur.root, alph=cur.alpha, reg=(sum(cur.delta.theta)+1), theta=mean(cur.theta))
				...
				if(runif(1)<probAlpha) {			# adjust alpha
					...
					if(runif(1)>(theta.rates.ratio/(theta.rates.ratio+1))) {
						if(runif(1)<0.2 | sum(cur.delta.rates)==0) {
							...
							nr=switcheroo(cur.delta.rates, cur.rates, ape.tre, theta=FALSE)
							new.alpha=cur.alpha		
							new.root.theta=cur.root.theta
							new.theta=cur.theta
							new.delta.theta=cur.delta.theta
						...
						if(runif(1)<0.2 | sum(cur.delta.theta)==0) {			# adjust theta
							...
							nr=switcheroo(cur.delta.theta, cur.theta, ape.tre, theta=TRUE)
							new.alpha=cur.alpha 
							new.theta=nr$new.values ##
							new.delta.theta=nr$new.delta ##
							new.root.theta<-nrt<-assign.root.theta(new.theta, ape.tre) ## 
...
#		cat(paste(i, sprintf("%.2f", mod.cur$lnL), sprintf("%.2f", mod.new$lnL), sprintf("%.2f", lnR), sQuote(decision), sep="\t\t"),"\n")
		...
		endparms=list(rates=cur.rates, cats=sum(cur.delta.rates)+1, root=cur.root, alph=cur.alpha, reg=(sum(cur.delta.theta)+1), theta=mean(cur.theta))
...
switcheroo <- function(cur.delta, cur.values, phy, theta=FALSE) { 
	bb=cur.delta
	vv=cur.values
	new.bb=sample(bb)
	...
	all.desc.anc=get.descendants.of.node(min(unlist(anc)), phy)
	...
	for(z in 1:length(new.shifts)) {
		...
		new.vv[match(get.descendants.of.node(node, phy), names(new.vv))]=new.values[z]
	} 
