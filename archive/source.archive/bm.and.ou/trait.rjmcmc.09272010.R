...
## BM IMPLEMENTATION ##
			...
			if(is.infinite(mod.new$lnL)) {
				pdf(file=paste(i,"failed.model.pdf",sep="."))
				plot(new.tre)
				dev.off()
				write.table(i,file="failed.rates.txt",append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
				write.table(matrix(new.rates,nrow=1),file="failed.rates.txt",append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
				write.table(matrix(c(i, new.root),nrow=1),file="failed.root.txt",append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
			}
		}
