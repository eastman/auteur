...
## OU IMPLEMENTATION ## 
		if(currModel=="OU") {
			...
			modCurr=new.ou.lik.fn(currRates, currAlpha, currRegimes, currTheta, currTree, ouch.dat, ordering)
			modNew=try(new.ou.lik.fn(newRates, newAlpha, newRegimes, newTheta, newTree, ouch.dat, ordering),silent=TRUE)
...
new.ou.lik.fn <- function(rates, alpha, regimes, theta, ape.tre, ouch.dat, ordering) { # from OUCH 2.7-1:::hansen()
