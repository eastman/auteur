...
fix.heritable.values<-function(bit.vector, values, phy, theta=FALSE) { # THE NEW SPLIT.OR.MERGE
	K=sum(bb)+1
	N=Ntip(phy)
...
# when direct descendants of the root node are chosen for split or merge, number of rates may exceed number of shifts in tree
	...
	if(sum(new.bb)>sum(bb)) { # new value shift: SPLIT
		print("split")
		if(theta) {
		...
		lnHastings=log((K+1)/(2*N-2-K)) # Drummond and Suchard 2010: where N is tips, K is number of local parms in tree
	} else { # drop a shift: MERGE
		print("merge")
		anc = get.ancestor.of.node(s, phy)
		if(length(anc)!=0 & !is.root(anc, phy)) {
			print(1)
		...
		lnHastings = log((2*N-2-K+1)/K) # Drummond and Suchard 2010: where N is tips, K is number of local parms in tree
	}
	old.vv=vv
	old.bb=bb
	bb=new.bb
	vv=new.vv
	if(sum(bb)!=(length(table(vv))-1)) {stop()}
}
#		shifts=sort(as.numeric(names(which(bb>0))))	
#		desc=lapply(shifts, function(x)get.descendants.of.node(x,phy))

#	   update value of corresponding node
#	   update all descendants of that node Unless subsequent rate shift
#	} else { # dropping old value shift: MERGE
#		s=names(new.bb[bb!=new.bb])
#		new.vv[which(names(new.vv)==s)] = new.vv[which(names(new.vv)==get.ancestor.of.node(s,phy))]
#		find ancestor of changed node and substitute ancestral value
#		find descendants of updated node and substitute with ancestral value (UNLESS subsequent rate shift)
#	}
