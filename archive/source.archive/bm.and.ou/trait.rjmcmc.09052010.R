...
rjMCMC.trait<-function (phy, data, ngen = 1000, model=c("OU","BM"), probAlpha=0.05, probRegimes=0.10, probMergeSplit=0.05, probShuffle=0.75, heat=1, fileBase = "result") 
{
...
### prepare objects for rjMCMC
	...
	currTheta [] <- mean(ouch.dat['trait'], na.rm=TRUE)		# initialize optimum under OU
...
## OU IMPLEMENTATION ## 
		if(currModel=="OU") {
			if (runif(1) < (2 * probMergeSplit)) {
				if(runif(1)<0.5) { # adjust regimes
					currTheta[]=get.theta(currRates, currAlpha, currRegimes, ape.tre, ouch.dat)
					nr=split.or.merge(currTheta, currRegimes, decide.s.or.m(currRegimes)->s.m)
		...
		summarize.gen(bundled.parms, currModel)
...
get.theta <- function(rates, alpha, regimes, apetree, ouch.dat) { # from OUCH 2.7-1:::hansen
	ouch.dat$regimes=regimes
	ouch.tre=apply.BMM.to.tree(rates, apetree)
	beta=ouch:::regime.spec(ouch.tre, ouch.dat['regimes'])
	dat=do.call(c,lapply(ouch.dat['trait'],function(y)y[ouch.tre@term]))
