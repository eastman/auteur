...
### prepare objects for rjMCMC
	...
	errorLog=file(paste(getwd(),"rjTraitErrors.log",sep="/"),open='w+')
	runLog=file(paste(getwd(),"rjTrait.log",sep="/"),open='w+')
	initmsg = paste("gen", "curr.lnL", "new.lnL", "lnR", sep="\t")
	write(initmsg, file=errorLog, append=TRUE)
	
### Begin rjMCMC
if(currModel=="OU")	cat("\tmodel\trates\talpha\t\tregimes\t\tlnL") else cat("\tmodel\trates\tlnL")
    for (i in 1:ngen) {
		error.flag=FALSE
		...
		if(currModel=="OU") {
			if (runif(1) < (2 * probMergeSplit)) {
				if(runif(1)<0.5) { # adjust regimes
					...
					if(s.m=="split") {
						nt=adjust.value(sample(currTheta, 1))
						newTheta=sample(c(nt,currTheta), size=max(newRegimes))
					} else {
						newTheta=sample(currTheta, size=max(newRegimes))
					}
					newRates=currRates
			...
			modNew=new.ou.lik.fn(newRates, newAlpha, newRegimes, newTheta, newTree, ouch.dat)
...
## BM IMPLEMENTATION ##
			...
			modNew=new.bm.lik.fn(newRates, newRoot, newTree, ouch.dat)
		}
		...
		if(i%%sampleFreq==0) summarize.gen(bundled.parms, currModel, runLog)
	}
...
summarize.gen<-function(parms, currModel, runLog) {
# LnL	rates	alpha	regimes
# 1		2		3		4
	if(currModel=="BM") {
		cat(msg<-paste("\n", sQuote(currModel), paste(parms[2], "\t\t", sprintf("%.3f", parms[1]),
				sep=""),sep="\t")
		)
	} else {
		cat(msg<-paste("\n", sQuote(currModel), paste(parms[2], "\t\t", sprintf("%.4f", parms[3]), "\t\t", parms[4], 
				"\t\t\t", sprintf("%.3f", parms[1]), sep=""),sep="\t")
		)
	}
	write(msg, file=runLog, append=T)
}
...
generate.error.message<-function(i, modCurr, modNew, lnR, errorLog) {
