...
# updated to adjust for an additional theta when regimes are split (or one fewer theta if regimes are merged)
	...
	new.cc=cc
	nn=length(vv)
	if(regimes) cm=2 else cm=1 # category multiplier to account for new (or fewer) thetas
	...
	if(task=="merge") {
		...
		lnProposalRatio = log(factor) + log(cm*nn) + log(cm*ncat) - log(numSplittableAfterMerge) - log(2^(n1 + n2) - 2) - log(abs(nr/mean(vv))) - log(n1 + n2)
...
#		lnPriorRatio = log(Stirling2(nn, ncat)) - log(Stirling2(nn, ncat - 1))
	...
	if(task=="split") {
		...
		lnProposalRatio = log(factor) + log(numSplittableBeforeSplit) + log(2^(n1 + n2) - 2) + log(abs(or/mean(vv))) + log(n1 + n2) - log(nn) - log(cm*(ncat + 1))
		lnPriorRatio = log(Stirling2(nn, ncat)) - log(Stirling2(nn, ncat + 1))
