...
rjMCMC.ou<-function (phy, data, ngen=1000, sampleFreq=100, probMergeSplit=0.05, probRoot=0.01, probRate=0.1, probAlpha=0.01, lambdaK=log(2), constrainK=FALSE, jumpsize=2, fileBase="result", simplestart=FALSE) 
{
#	load("rjMCMC/versions/fakedata.rda"); phy=sims$phy; data=sims[[2]]; ngen=1000; sampleFreq=100; probMergeSplit=0.05; probRoot=0.01; probRate=0.1; probAlpha=0.01; lambdaK=log(2); constrainK=FALSE; jumpsize=2; fileBase="result"; simplestart=FALSE 
...
# initialize parameters
	...
	cur.alpha		<- adjust.rate(0.01, jumpsize)
...
## OU IMPLEMENTATION ##
		...
		if(inherits(mod.new, "try-error")) {
			mod.new=as.list(mod.new)
			mod.new$lnL=Inf
			lnLikelihoodRatio = -Inf
			failures=paste("./failed", model, "parameters/",sep=".")
			if(!file.exists(failures)) dir.create(failures)
			failed.theta=new.theta
			names(failed.theta)=ape.tre$edge[,2]
			write.table(matrix(c(i, new.rate),nrow=1),file=paste(failures,"failed.rates.txt",sep="/"),append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
			write.table(matrix(c(i, new.root.theta),nrow=1),file=paste(failures,"failed.root.txt",sep="/"),append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
			write.table(matrix(c(i, new.alpha),nrow=1),file=paste(failures,"failed.alpha.txt",sep="/"),append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
			write.table(matrix(c(i, failed.theta),nrow=1),file=paste(failures,"failed.theta.txt",sep="/"),append=TRUE,col=FALSE,row=FALSE,quote=FALSE)
		} else {
			lnLikelihoodRatio = mod.new$lnL - mod.cur$lnL
		}	
...
#	phy; base.dir; burnin=0; level=0.03; internal.only=FALSE; paint.branches=TRUE; pdf.base=NULL; verbosity.base=NULL
	nullphy=phy
	nullphy$tip.label[]=""
	color.length=21
	require(ape)
...
# send to pdf 
	...
	legend("topright", title=toupper("direction"), cex=0.5, pt.cex=1, text.col="darkgray", legend = sprintf("%6.1f", rev(c.seq[seq(1,length(c.seq),by=2)])), pch=21, ncol=1, col = "darkgray", pt.bg = rev(ccx[seq(1,length(c.seq),by=2)]), box.lty="blank", border="white")
	if(length(nodes) | (length(aa.tips) & !internal.only)) {
		legend("bottomright", title=toupper("frequency"), cex=0.5, pt.cex=2*(seq(min(c(cc,tt.cex)), max(c(cc,tt.cex)), length=6)), text.col="darkgray", legend = sprintf("%10.3f", seq(round(min(c(aa,aa.tips)),2), round(max(c(aa,aa.tips)),2), length=6)), pch=21, ncol=1, col = "darkgray", pt.bg = "white", box.lty="blank", border="white")
	}
