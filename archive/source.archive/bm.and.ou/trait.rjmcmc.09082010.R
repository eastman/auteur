...
### prepare objects for rjMCMC
	...
	generate.log(parms=NULL, currModel=currModel, runLog=runLog, init=TRUE)
...
## BM IMPLEMENTATION ##
		...
		if(i%%sampleFreq==0) generate.log(bundled.parms, currModel, runLog)
	}
...
generate.log<-function(parms, currModel, runLog, init=FALSE) {
# LnL	rates	alpha	regimes
# 1		2		3		4
	if(init) {
		if(currModel=="BM") msg=paste("model", "rates", "lnL", sep="\t") else msg=paste("model", "rates", "alpha", "regimes", "lnL", sep="\t")
	} else {
		if(currModel=="BM") {
			msg<-paste(sQuote(currModel), parms[2], sprintf("%.3f", parms[1]),sep="\t")
		} else {
			msg<-paste(sQuote(currModel), parms[2], sprintf("%.4f", parms[3]), parms[4], sprintf("%.3f", parms[1]),sep="\t")
		}
	}
