...
#	currTheta[] <- mean(td$data[,1])		# initialize optima
	...
	lowAlpha <- 0.000001					# set lower limit on alpha 
...
# Begin rjMCMC
    ...
    for (i in 1:ngen) {
		...
		if(runif(1) < changeModel) {
			if(currModel=="BM") {
				if(currAlpha>lowAlpha) {
					currModel="OU" 
				} else {
					if(runif(1)>0.5) currModel="OU" else currModel="BM"
				}
			} else currModel="BM"
		}
...
lnl.BM<-function(rates, apetree, ouch.dat) {
	ouchtree=apply.rates.to.tree(rates, apetree)
	dat=do.call(c,lapply(ouch.dat['trait'],function(y)y[ouchtree@term]))
	nterm <- ouchtree@nterm
	nchar <- 1
	w <- matrix(data=1,nrow=nterm,ncol=1)
	b <- ouchtree@branch.times
