...
## OU IMPLEMENTATION ## 
		if(currModel=="OU") {
			...
			modCurr=new.ou.lik.fn(currRates, currAlpha, currRegimes, currTheta, ouch.tre, ouch.dat)
			modNew=new.ou.lik.fn(newRates, newAlpha, newRegimes, newTheta, ouch.tre, ouch.dat)
...
## BM IMPLEMENTATION ##
			...
			modCurr=new.bm.lik.fn(currRates, currRoot, ouch.tre, ouch.dat)
			modNew=new.bm.lik.fn(newRates, newRoot, ouch.tre, ouch.dat)
		}
...
new.ou.lik.fn <- function(rates, alpha, regimes, theta, ouch.tre, ouch.dat) { # from OUCH 2.7-1:::hansen()
	ouch.dat$regimes=as.factor(regimes)
	ouch.tre=apply.BMM.to.ouchtree(ouch.tre, rates)
...
new.bm.lik.fn <- function(rates, root, ouch.tre, ouch.dat) { # from OUCH 2.7-1:::brown()
	ouch.tre=apply.BMM.to.ouchtree(ouch.tre, rates)
	dat=do.call(c,lapply(ouch.dat['trait'],function(y)y[ouch.tre@term]))
