...
## SUMMARIZING MCMC ##
assess.parm.estimates=function(phy, results.file, level=0.03, plot=TRUE) {
	results=read.table(results.file)
	...
	desc=lapply(nodes, function(x) {foo=get.descendants.of.node(x,phy); return(phy$tip.label[foo[foo<=Ntip(phy)]])})
	shifts=apply(results, 1, sum)
	if(plot) {
		plot(phy, cex=0.2, lwd=0.4)
		nn=(Ntip(phy)+1):nrow(phy$edge)
		ll<-cc<-rep(0,length(nn))
		ll[nn%in%nodes]=1
		cc[nn%in%nodes]=aa[match(nn[nn%in%nodes],nodes)]/max(aa)
		nodelabels(pch=ifelse(ll==1, 21, NA), bg=ifelse(ll==1, "black", "white"), cex=2*cc)

	}
	return(list(shift.prop=aa, desc=desc, mean.shifts=mean(shifts), median.shifts=median(shifts)))
}
