...
## OU IMPLEMENTATION ## 
		if(currModel=="OU") {
			...
			modCurr=lnl.OU(currRates, currAlpha, currRegimes, ouch.tre, ouch.dat)
			modNew=lnl.OU(newRates, newAlpha, newRegimes, ouch.tre, ouch.dat)
...
## BM IMPLEMENTATION ##
			...
			modCurr=lnl.BM(currRates, ouch.tre, ouch.dat)
			modNew=lnl.BM(newRates, ouch.tre, ouch.dat)
		}
...
lnl.OU <- function(rates, alpha, regimes, ouch.tre, ouch.dat) { # from OUCH 2.7-1:::hansen
...
lnl.BM <- function(rates, ouch.tre, ouch.dat) { # from OUCH 2.7-1:::brown
	ouch.tre=apply.BMM.to.ouchtree(ouch.tre, rates)
	dat=do.call(c,lapply(ouch.dat['trait'],function(y)y[ouch.tre@term]))
