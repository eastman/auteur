...
## OU currently not implemented (JM Eastman 09.2010), but multiBMM working
...
### JM Eastman 09.2010: prepare data for OUCH
...
# attempt OU-modeling with OUCH
	# get BETA from regimes
	ouch.dat$regimes=currRegimes
	beta=ouch:::regime.spec(ouch.tre, ouch.dat['regimes'])
	
	# convert data into appropriate object 
	dat=do.call(c,lapply(ouch.dat['trait'],function(y)y[ouch.tre@term]))
	
	# get deviance under OU (returning correct LnL if input 'optimal' currRate and currAlpha for OUCH documentation)
	if(currAlpha>0) {
		ou.lnL=-0.5*ouch:::ou.lik.fn(ouch.tre, as.matrix(currAlpha), as.matrix(currRate), beta, dat)$deviance	
	}
