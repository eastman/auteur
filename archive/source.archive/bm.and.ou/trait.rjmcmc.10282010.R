## Written by LJ HARMON, AL HIPP, and JM EASTMAN:	2008-2010
...
# beta: not dependent on ultrametricity -- map regimes relative to *topology*
	...
	w <- .Call(ouch:::ouch_weights,object=ouch.tre,lambda=ev$values,S=ev$vectors,beta=beta) # DEPENDENT UPON ULTRAMETRICITY
...
## SUMMARIZING MCMC ##
shifts.plot=function(phy, base.dir, optima=FALSE, burnin=0, level=0.03, paint.branches=TRUE, pdf.base=NULL, ...) {
	oldwd=getwd()
	setwd(base.dir)
	files=dir()
	if(optima) {
	...
	if(!all(names(results)%in%phy$edge[,2])) stop("Cannot process results")
	hits=apply(results,1,sum)
	hits=length(hits[hits>1])
	aa=apply(results, 2, function(x) sum(x))
	aa=aa/hits
	aa=aa[aa>=level]
	aa=aa[order(aa, decreasing=TRUE)]
	aa=aa[which(as.numeric(names(aa))>Ntip(phy))]
	...
	if(paint.branches) {
		require(colorspace)
		average.ests=apply(ests,2,mean)
		cce=diverge_hcl(21, c = c(100, 0), l = c(50, 90), power = 1.1)
		e.seq=seq(min(average.ests),max(average.ests),length=21)
		colors.branches=sapply(average.ests, function(x) cce[which(min(abs(e.seq-x))==abs(e.seq-x))])
...
# send to pdf 
	...
	plot(phy, cex=0.2, lwd=0.4, edge.width=0.5, edge.color=colors.branches, ...)
	...
	if(is.null(colors.nodes)) {
		...
		nodelabels(pch=ifelse(ll==1, 21, NA), cex=2*cc, col=1, bg=rr, lwd=0.5)
	...
	return(list(desc=desc, mean.shifts=mean(shifts), median.shifts=median(shifts), shift.prop=aa, shift.direction=cols))
}
