...
shifts.plot <-
function(phy, base.dir, burnin=0, level=0.01, internal.only=FALSE, fan=FALSE, paint.branches=TRUE, legend=TRUE, verbose=TRUE, lab="OUT", ...) {
	color.length=17
	oldwd=getwd()
	setwd(base.dir)
...
#	if(pdf) pdf(file=paste(outdir, paste(out.base,"pdf",sep="."),sep="/"))
	if(!fan){
		...
		plotfangram(phy, cex=CEX, lwd=0.4, edge.color=colors.branches, show.tip.label=TRUE, label.offset=strwidth(par("pch"), cex=1.25), no.margin=TRUE, default=list(col=colors.branches[round(length(colors.branches)/2)],lwd=0.4),...)
