...
function(phy, dat, nsteps=100, model=c("BM","LEVY","OU"), widths=NULL, lim=list(min=0, max=1000)) {
	if(!withinrange(nsteps, 100, 1000)) {
		if(nsteps>1000) nsteps=1000 else nsteps=100
	}
	if(!length(model)) stop("Please specify model")
	if(length(model)>1) stop("Please specify a single model")
