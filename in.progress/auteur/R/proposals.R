...
#rjmcmc and mcmc proposal mechanism: locally select an edge based on subtended or subtending branch lengths
#author: JM EASTMAN 2011

tree.slide <-
function(phy, node=NULL, up=NA, use.edges=FALSE){
	if(is.na(up)) up=as.logical(round(runif(1)))
	
	# choose random node if none given
	if(is.null(node)) {
		ee=cumsum(phy$edge.length)/sum(phy$edge.length)
		names(ee)=phy$edge[,2]
		node=as.numeric(names(ee)[min(which(runif(1)<ee))])
		return(list(node=node, direction=NA))
	}
	...
	if(up){
		...
		if(!length(descendants)) {		# -- choice is a tip
			if(runif(1)<0.5) {
				...
				choice=tree.slide(phy, node, up=TRUE, use.edges=use.edges)$node
				direction="up"
			}
...
adjustjump=function(jump.table, jVCV, add=FALSE, drop=FALSE, swap=FALSE, phy, node.des){
	
	if(add==TRUE){												# add new jump point
		
		jj=tree.slide(phy, node=NULL)$node
		jump.table=c(jump.table, jj)
		jVCV=updatevcv.jumps(jVCV, from=NULL, to=jj, node.des)
		todo="add"
		...
		if(drop==TRUE & length(jump.table)) {					# drop jump point
			
			jump.table=jump.table[-adjustedjump.index]
			jVCV=updatevcv.jumps(jVCV, from=adjustedjump.loc, to=NULL, node.des)
			todo="drop"
			
		} else if(swap==TRUE & length(jump.table)){				# add new jump point
			
			adjustedjump.newloc<-ajnl<-tree.slide(phy, node=adjustedjump.loc, up=NA, use.edges=FALSE)$node
			jump.table[adjustedjump.index]=adjustedjump.newloc		
			if((from<-adjustedjump.loc) != (to<-adjustedjump.newloc)) {
				jVCV=updatevcv.jumps(jVCV, from=from, to=to, node.des)
			} else {
				jVCV=jVCV
			}
			todo="swap"
...
#ensures values are all within bounds
#author: JM EASTMAN 2011
...
#computes VCV matrix under Poisson-jump process
#author: JM EASTMAN 2011

updatevcv.jumps=function(jVCV, from=NULL, to=NULL, node.des){
...
#combines VCV matrices from Poisson-jump and diffusion processes
#author: JM EASTMAN 2011

updatevcv.levy=function(vcvB, vcvJ, sigsqB, sigsqJ){
	return(sigsqB*vcvB+sigsqJ*vcvJ)
}



#computes array of jumps at each branch in a phylogeny
#author: JM EASTMAN 2011
