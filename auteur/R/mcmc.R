...
mcmc.levy <- function (	phy, dat, SE=0, ngen=1000, sample.freq=100, 
						...
						prop.width=NULL, lambdaN=log(2), summary=TRUE, fileBase="result") 
{ 
	
#	require(auteur); tmp=phenogram(rcoal(200), "levy", plot=FALSE); phy=tmp$phy; dat=tmp$dat; prob.lambda=0.75; prob.sigsqB=0.075; prob.sigsqJ=0.15; lambdaN=log(2); prop.width=0.5; summary=TRUE; fileBase="result"; ngen=1000; sample.freq=100; SE=0
	model="jumpBM"
...
# jump table
	...
	if((nj<-rpois(1, cur.lambda)>0)) {
		for(i in 1:nj) {
			cur.jump.table[nj]=c(cur.jump.table, to<-tree.slide(ape.tre))
			cur.jVCV=updateJvcv(cur.jVCV,from=NULL,to=to,node.des)
		}
	}
	cur.nJ			<- length(cur.jump.table)
	cur.lVCV		<- updateLvcv(vcv, cur.jVCV, cur.sigsqB, cur.sigsqJ)

# compute lnL for starting state
	mod.cur = bm.lik.fn(cur.root, orig.dat, cur.lVCV, SE)
	cur.lnL=mod.cur$lnL
...
## jump-diffusion IMPLEMENTATION ##
		while(1) {
			...
			if (cur.proposal==1) {													## adjust JUMP parameters ##
				if(runif(1)>0.80 ) {													# adjust lambda
					new.lambda=adjustrate(cur.lambda, prop.width)
					...
					tmp=adjustjump(cur.jump.table, cur.jVCV, kLoc, rLoc, add=FALSE, drop=FALSE, swap=TRUE, ape.tre, node.des)
					...
					if(cur.nJ==0) {
						if(runif(1)<0.5) {													# -- 0 -> 1
							new.nJ=1
							tmp=adjustjump(cur.jump.table, cur.jVCV, kLoc, rLoc, add=TRUE, drop=FALSE, swap=FALSE, ape.tre, node.des)
							new.jump.table=tmp$jump.table
							new.jVCV=tmp$jVCV	
							subprop="increment"
						} else {															# -- 0 -> 0
						...
						if(runif(1)<0.5){													# -- j -> j + 1
							...
							new.nJ=cur.nJ-1
							tmp=adjustjump(cur.jump.table, cur.jVCV, kLoc, rLoc, add=FALSE, drop=TRUE, swap=FALSE, ape.tre, node.des)
							new.jump.table=tmp$jump.table
							new.jVCV=tmp$jVCV
							subprop="decrement"
						}
				...
				new.lambda=cur.lambda
				new.sigsqB=adjustrate(cur.sigsqB, prop.width)
				new.sigsqJ=cur.sigsqJ
				new.root=cur.root
				new.jVCV=cur.jVCV
				new.nJ=cur.nJ
				...
				new.lambda=cur.lambda
				new.sigsqB=cur.sigsqB
				new.sigsqJ=adjustrate(cur.sigsqJ, prop.width)
				new.root=cur.root
				new.jVCV=cur.jVCV
				new.nJ=cur.nJ
				new.jump.table=cur.jump.table
				...
				new.sigsqB=cur.sigsqB
				new.sigsqJ=cur.sigsqJ
				new.root=adjustvalue(cur.root,prop.width)
				new.jVCV=cur.jVCV
				new.nJ=cur.nJ
				new.jump.table=cur.jump.table
				subprop="root.state"
		...
		if(any(new.jVCV<0)) stop("Broken updating of jump matrix.")
		
		n.props[cur.proposal]=n.props[cur.proposal]+1				

	# compute fit of proposed model
		if(subprop!="root.state") new.lVCV=updateLvcv(vcv, new.jVCV, new.sigsqB, new.sigsqJ) else new.lVCV=cur.lVCV
		mod.new=NULL
		mod.new=try(bm.lik.fn(new.root, orig.dat, new.lVCV, SE), silent=TRUE)
